...
float justSDF(in vec3 point) {
    ...
    vec3 shiftEye = vec3((consts.mousePos - consts.iResolution / 2.0) * vec2(1 , -1) * 0.00005, 0);

    SDFs[0] = sdPyramid(point - vec3(0, -0.5, 2) + shiftEye * 0.5, 1.0) - 0.05;
    
    SDFs[1] = sdCappedCylinder(point - vec3(0, 0.75, 2) + shiftEye * 0.5, 0.2, 0.1) - 0.01;
    
    SDFs[2] = sdCappedCylinder(point - vec3(0, 0.55, 2) + shiftEye * 0.5, 0.01, 0.2) - 0.01;
    
    vec3 shiftIris = shiftEye * 2.0;
    vec3 eye = vec3(0, -0.15, 1.5);
    SDFs[3] = sdVesicaSegment(point, eye + shiftEye + vec3(-0.2, 0, 0), eye + shiftEye + vec3(+0.2, 0, 0), 0.12) - 0.01;
    
    SDFs[4] = sdVesicaSegment(point, eye + shiftIris + vec3(0, -0.08, -0.14), eye + shiftIris + vec3(0, +0.1, -0.14), 0.025) - 0.01;
...
bool castRay(in vec2 uv, inout vec4 surfCol, inout vec3 point) {
    ...
    while(length(point) < 1000.0) {
        // Using iResolution here to ensure that compiler doesn't optimize the loop away.
        float workload = 0.0;
        for (int i = 0; i < int(consts.iResolution.x * 1.6 * 0.5); i++) {
            workload += sin(float(i)) * cos(float(i));
        }
        ...
        point += ray * sdf + vec3(1) * workload * 1e-6;
    }
