  ...
  , useVsync{true}
{
  ...
  etna::create_program("mainProgram", {INFLIGHT_FRAMES_SHADERS_ROOT "v_shader.vert.spv", 
    INFLIGHT_FRAMES_SHADERS_ROOT "toy.frag.spv"});
  etna::create_program("textureProgram", {INFLIGHT_FRAMES_SHADERS_ROOT "v_shader.vert.spv", 
    INFLIGHT_FRAMES_SHADERS_ROOT "texGen.frag.spv"});
  ...
  constsUniformBuffers.emplace(
    etna::get_context().getMainWorkCount(),
    [](std::size_t) {
      return etna::get_context().createBuffer(etna::Buffer::CreateInfo{  // mapping is lower
        .size = sizeof(sizeof(float) + sizeof(glm::vec2) + sizeof(glm::ivec2)),  // time, resolution, mouse
        .bufferUsage = vk::BufferUsageFlagBits::eUniformBuffer,
        .memoryUsage = VMA_MEMORY_USAGE_CPU_ONLY,
        .name = "consts",
      });
  });
  constsUniformBuffers.value().iterate([](etna::Buffer& b){
    b.map();
  });
}
...
void App::drawFrame()
{
  ...
  if (nextSwapchainImage)
  {
    ...
    {
      ETNA_PROFILE_GPU(currentCmdBuf, "FrameDraw");
      {
        ZoneScopedN("SleepingAtTheJob_8ms");
        std::this_thread::sleep_for(std::chrono::milliseconds(8));
      }

      {
        ETNA_PROFILE_GPU(currentCmdBuf, "draw_gen_texture");
        // ZoneScopedN("draw_gen_texture");
        ...
        {
          ETNA_PROFILE_GPU(currentCmdBuf, "imgGenScope");
      ...
      {
        ETNA_PROFILE_GPU(currentCmdBuf, "justDrawFrame");
        // ZoneScopedN("justDrawFrame");
        ...
        pushConsts.mouse = osWindow->mouse.freePos;
        std::memcpy(constsUniformBuffers.value().get().data(), &pushConsts, sizeof(pushConsts));
        ...
        {
          ETNA_PROFILE_GPU(currentCmdBuf, "mainProgramScope");
          ...
          auto s = etna::create_descriptor_set(
            etna::get_shader_program("mainProgram").getDescriptorLayoutId(0), currentCmdBuf, {
              ...
              etna::Binding{2, constsUniformBuffers.value().get().genBinding()}
          });
